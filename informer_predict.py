...
from pipeline_informer.data_loader import Dataset_ETT_hour, Dataset_SHARE_hour_test, Dataset_ETT_minute, \
    Dataset_SHARE_minute
...
def predict(freq='h', features='S', patience=7, use_technical_indicator=False,
    ...
    # 下载2天的数据
    datetime_date_end = pipeline_utils.datetime.get_datetime_from_date_str(date_end)

    date_start = datetime_date_end
    for i in range(60):
        date_start = pipeline_utils.datetime.get_next_work_day(date_start, next_flag=-1)
        pass
    ...
    if add_predict_data is True:
        ...
        for i in range(predict_days):
            ...
            if date_predict_temp is None:
                date_predict_temp = pipeline_utils.datetime.get_datetime_from_date_str(date_end)
            pass

            next_date_predict_temp = pipeline_utils.datetime.get_next_work_day(date_predict_temp, next_flag=+1)
